...
def import_info_return(project_name, method_name):
    # dir = "source_file_parser" + '/' + project_name + '_out.json'
    # with open(dir, 'r') as f:
    #     data = json.load(f)
    # 使用 Path 对象安全构建跨平台路径
    source_path = "xxx"
    dir_path = Path(source_path) / "source_file_parser" / f"{project_name}_out.json"

    # 安全读取文件（自动处理路径分隔符）
    try:
        with open(dir_path, 'r', encoding='utf-8') as f:  # 明确指定编码
            data = json.load(f)
    except FileNotFoundError:
        raise FileNotFoundError(f"文件未找到: {dir_path}")
    for item in data:
...
def repair_error(generate_content, package_name, class_name, import_info: list, method_name: str):
    """
        ...
        if not match_class:
            ...
            if match_annotation:
                ...
                if match_method:
                    ...
                    repair_code = add_method_framework_and_class_framework(generate_content, package_name, class_name,
                                                                           method_name)
        else:
